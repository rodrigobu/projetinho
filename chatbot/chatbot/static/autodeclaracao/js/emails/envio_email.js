...
var valida_configuracao = function(){
	var to_return = true;
	$.ajax({
	    url      : URL_VALIDA_CONFIG,
		type     : 'get',
		dataType : 'json',
		async    : false,
		data     : { 'tipo_envio' : $("[name='tipo']:checked").val() },
		success  : function(dados) {
		    if (dados['status'] == 'nok') {
			    $.dialogs.error(dados["msg"]);
			    to_return = false;
...
$(function() {
	...
	$('#submit-id-avancar').on('click', function() {
		if(!valida_configuracao()){
			return false;
		}else if ($(".checkbox_colab:checked").length == 0) {
