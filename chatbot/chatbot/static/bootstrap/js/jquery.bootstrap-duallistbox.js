;(function ($, window, document, undefined) {
  ...
  function move(dualListbox) {
    ...
    if(dualListbox.settings.sortOptionsOnSelect){
  ...
  function removeAll(dualListbox) {
    ...
    dualListbox.element.find("option[disabled!='disabled']").each(function(index, item) {
      var $item = $(item);
      if (!$item.data('filtered2')) {
        if ($item.val() == 'funcao' || $item.val() == 'nome'){
          $item.prop('selected', true);
